#Fine-tune script for RobBERT Dutch model to output arousal and valence scores
import pandas as pd
import numpy as np
...
words_arousal_df = pd.read_excel('~/All_Arousal.xlsx', sheet_name='MeanArousalPerWord') #Speed Dutch dataset
words_arousal_df = words_arousal_df[['Word', 'Arousal']]
min_ar = words_arousal_df['Arousal'].min()
max_ar = words_arousal_df['Arousal'].max()
words_arousal_df['Arousal'] = (words_arousal_df['Arousal'] - min_ar) / (max_ar - min_ar)

words_valence_df = pd.read_excel('~/All_Valence.xlsx', sheet_name='Means') #Speed Dutch dataset
words_valence_df = words_valence_df[['Word', 'Valence']]
min_val = words_valence_df['Valence'].min()
max_val = words_valence_df['Valence'].max()
words_valence_df['Valence'] = (words_valence_df['Valence'] - min_val) / (max_val - min_val)
...
#Merge word datasets
words_df = pd.merge(words_arousal_df, words_valence_df, on='Word', how='inner')
...
print("Pre-training on words:")
word_train, word_eval = train_test_split(words_df, test_size=0.1, random_state=42)
word_train_dataset = ArousalValenceDataset(word_train['Word'].tolist(), word_train['Arousal'].tolist(), word_train['Valence'].tolist(), tokenizer)
word_eval_dataset = ArousalValenceDataset(word_eval['Word'].tolist(), word_eval['Arousal'].tolist(), word_eval['Valence'].tolist(), tokenizer)

word_model = RobertaForSequenceClassification.from_pretrained(
    "DTAI-KULeuven/robbert-2023-dutch-base",
    num_labels=2,
    problem_type="regression"
)

word_training_args = TrainingArguments(
    output_dir='~/ArValWordSent/word_results',
    num_train_epochs=3,
    per_device_train_batch_size=32,
    per_device_eval_batch_size=64,
    warmup_steps=500,
    weight_decay=0.01,
    logging_dir='~/ArValWordSent/word_logs',
    logging_steps=10,
    #save_steps=100,
    save_strategy='no',
    eval_steps=10,
    evaluation_strategy="steps",
    load_best_model_at_end=False, #True if save_strategy="yes"
)

word_trainer = CustomTrainer(
    model=word_model,
    args=word_training_args,
    train_dataset=word_train_dataset,
    eval_dataset=word_eval_dataset,
)

word_trainer.train()
plot_losses(trainerobj=word_trainer, fold='words')

word_model.save_pretrained("~/ArValWordSent/word_model")
...
for fold, (train_idx, val_idx) in enumerate(kf.split(train_cv_df)):
    ...
    model = RobertaForSequenceClassification.from_pretrained("~/ArValWordSent/word_model")
